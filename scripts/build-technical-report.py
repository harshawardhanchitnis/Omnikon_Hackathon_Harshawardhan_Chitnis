...
# Executive summary
story += section("01 / Executive summary", "A complete teacher workflow, not a text generator.", "ChalkBox reduces lesson-planning friction for teachers who have limited time, materials, connectivity, and support. It creates a strong starting point and follows the lesson into the classroom.")
story += [
    P("The challenge", "H2x"),
    ...
    data_table(
        ...
        [
            ...
            ["Classroom continuity", "Plan remains useful after preparation", "Teach Mode, timer, notes, reflection"],
    ...
    Spacer(1, 5 * mm),
    callout("Core outcome", "A teacher moves from a blank page to a classroom-ready, editable lesson with measurable objectives, timed low-resource activities, assessment evidence, and a reusable reflection loop."),
    Spacer(1, 5 * mm),
...
story += section("02 / Product scope", "Plan, teach, reflect, improve.", "The main journey is intentionally linear for a first-time teacher and modular for a returning teacher.")
story += [
    flow_table([
        ("Brief", "Grade, topic, time, language, class and constraints"),
        ("Plan", "Gemini or labelled prepared example"),
        ("Edit", "Objectives, sequence, assessment and sources"),
        ("Teach", "Timer, steps, offline options and notes"),
        ("Reflect", "Class-level evidence and next action"),
    ...
    data_table(
        ...
        [
            ...
            ["Teacher home", "Next lesson, recent plans, outcome signal, mentor review, planning metrics"],
            ["Plan creation", "Board, grade, subject, language, duration, class size, materials, constraints, level"],
            ["Editor", "Nested editing, add/remove/reorder, autosave, status, deterministic quality checks"],
            ["Classroom document", "Preview, print, PDF, share, disclosure and attribution"],
            ["Teach and reflect", "Timer, pause, step map, quick notes, outcome, rating and next action"],
            ["Reuse and support", "Search/filter, duplicate, community adaptation, mentors, appointment, guidance"],
            ["Insights", "Created/taught rhythm, subject mix, time estimate, outcome and evidence loop"],
            ["Administration", "Health, teachers, content provenance, quota and validation telemetry"],
        ],
        [39 * mm, 127 * mm],
    ),
    Spacer(1, 5 * mm),
    callout("Explicit boundary", "There are no student accounts, named learner records, automatic individual grading, payments, native apps, or silent AI fallbacks.", SUN_LIGHT),
    PageBreak(),
]
...
story += [
    ...
    data_table(
        ...
        [["Meera Patil", "Grade 5-7 teacher at a fictional Zilla Parishad school in Pune district", "3 plans, 1 taught session, check-in, reflection, notifications, mentor support, analytics"]],
        [33 * mm, 66 * mm, 67 * mm],
    ),
    Spacer(1, 5 * mm),
    ...
    data_table(
        ...
        [
            ["The Water Cycle Around Us", "Ready", "38 learners, bilingual, no projector", "Full edit, quality, teach, PDF and reflection flow"],
    ...
    bullet("Prepared plans carry a visible purple label in creation, editor, and preview."),
    ...
    bullet("Analytics are calculated from the fixture records, not hard-coded display totals."),
    Spacer(1, 4 * mm),
    P("Recommended five-minute path", "H2x"),
    flow_table([
        ("Land", "State the time/material problem"),
        ("Home", "Show next lesson and evidence"),
        ("Create", "Load the prepared example"),
        ("Teach", "Run steps and save a note"),
        ("Insight", "Reflect and show the loop"),
    ], [29 * mm, 5 * mm, 29 * mm, 5 * mm, 29 * mm, 5 * mm, 29 * mm, 5 * mm, 29 * mm]),
    Spacer(1, 6 * mm),
...
story += [
    data_table(
        ...
        [
            ...
            ["State", "Zustand + TanStack Query", "Small application store plus server-query foundation"],
    ...
    P("Data flow", "H2x"),
    flow_table([
        ("UI", "Validated domain action"),
        ("Store", "Immediate state update"),
        ("IndexedDB", "Local write first"),
        ("Postgres", "RLS upsert when online"),
    ], [36 * mm, 7 * mm, 36 * mm, 7 * mm, 36 * mm, 7 * mm, 36 * mm]),
    Spacer(1, 5 * mm),
    callout("Resilience rule", "A cloud failure cannot erase the teacher's local work. Registered sync is additive; demo data never leaves the browser."),
    PageBreak(),
]

# AI
story += section("05 / Responsible AI and retrieval", "Generate structure. Preserve judgement.", "The model receives a bounded classroom brief and trusted retrieval context through a protected Edge Function.")
story += [
    flow_table([
        ("Auth", "Verify user and owner"),
        ("Quota", "5 demo / 25 teacher daily"),
        ("RAG", "Approved 768-d vector matches"),
    ...
    data_table(
        ...
        [
            ...
            ["Source trust", "Only approved chunks with licence and attribution", "Inspectible source cards"],
    ...
    Spacer(1, 5 * mm),
    P("Teacher review remains mandatory for current syllabus fit, facts, language, safety, accessibility, and the needs of the actual class.", "Bodyx"),
    PageBreak(),
]
...
story += [
    data_table(
        ...
        [
            ...
            ["Lesson plan", "Content, quality, sources, status, sharing", "Owner CRUD; public read only when published"],
            ["Teaching session", "Timer, current step, class-level notes", "Owner only"],
            ["Check-in", "Understanding, engagement, pace, evidence", "Owner only"],
            ["Reflection", "What worked, change, outcome, next action", "Owner only"],
            ["Support", "Experts, appointment, guidance, tasks", "Teacher or explicit admin policy"],
    ...
    bullet("Environment files, provider keys, service keys, build output and local evidence are excluded from Git."),
    Spacer(1, 5 * mm),
...
story += [
    ...
    Spacer(1, 5 * mm),
    callout("Offline truth", "AI generation and cloud sync require connectivity. Saved plans, editing, Teach Mode, reflection, print, and prepared demo remain available from the installed PWA."),
    PageBreak(),
]
...
story += [
    data_table(
        ...
        [
            ["Innovation and creativity", "Full evidence loop, low-resource constraint engine, inspectable AI quality, explicit prepared-demo integrity"],
            ...
            ["User experience", "Responsive shell, quick create, autosave, Teach Mode, actionable states, resettable demo"],
    ...
    data_table(
        ...
        [
            ...
            ["Stops after generation", "Edit, preview, teach, assess, reflect and reuse"],
            ...
            ["Demo breaks when API fails", "Prepared deterministic journey with explicit label"],
...
story += [
    data_table(
        ...
        [
            ...
            ["Unit/component", "Quality, analytics, fixtures, plan card", "8 tests pass"],
            ["Production build", "Code splitting, PWA manifest and service worker", "Pass"],
            ["Browser lifecycle", "Prepared journey across desktop and Pixel 7", "Playwright suite included; CI gate"],
...
story += [
    data_table(
        ...
        [
            ...
            ["Gemini", "Lesson JSON and embeddings", "Free-tier models; 5/25 daily application quotas"],
    ...
    P("1. Apply Supabase migrations and confirm RLS.  2. Set Edge Function secrets.  3. Deploy generation, indexing and health functions.  4. Connect GitHub to Cloudflare Pages.  5. Add public build variables.  6. Set final Auth callback URLs.  7. Enable Turnstile after hostname verification.  8. Run incognito, offline, share and role checks.", "Numberx"),
    Spacer(1, 5 * mm),
    ...
    bullet("Source, production build, migrations, functions, tests, CI, documentation and ZIP are present."),
    ...
    bullet("Final hosted verification covers desktop, mobile, offline, auth, live AI, PDF, share and roles."),
    PageBreak(),
]
...
story += [
    ...
    Spacer(1, 6 * mm),
    callout("Build", "A complete, responsive, installable teacher workspace with a deterministic judge path and a production-like cloud path."),
    Spacer(1, 4 * mm),
    callout("Innovate", "Constraint-aware structured generation, attributable retrieval, inspectable quality and the plan-to-reflection evidence loop.", SUN_LIGHT),
    Spacer(1, 4 * mm),
